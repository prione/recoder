...
class recorder():
    def __init__(self):
        ...
        self.r_button_speaker = tk.Radiobutton(frame1,value=0,variable=self.radio_val,text="スピーカー", command=self.set_combo_device)
        self.r_button_mic = tk.Radiobutton(frame1,value=1,variable=self.radio_val,text="マイク", command=self.set_combo_device)
    ...
    def rec_start(self):
        ...
        def get_recorded_data(self):
            ...
            if recorded_data != []:
                data = np.reshape(np.concatenate(recorded_data), [-1, len(set(r.channelmap))])
    ...
    def rec_stop(self):
        ...
        try:
            ...
            tk.messagebox.showinfo("完了",f"{self.filename}で保存しました")
        except:
            pass
    ...
    def close_main(self): 
        if self.is_saving:
            tk.messagebox.showerror("エラー","ファイル保存中です。")
        else:
            self.root.destroy()
